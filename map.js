...
function initGoogleMap(user = {}) {
    const defaultCenter = { lat: 43.5539, lng: 7.0170 };
    ...
    vendors.forEach((v, idx) => {
        ...
        <div style="display:flex;align-items:center;gap:10px;">
            ${v.logo ? `<img src="${v.logo}" alt="${v.name} logo" style="width:40px;height:40px;border-radius:6px;">` : ''}
            <div>
                <b>${v.name}</b><br>${v.type}<br>
                <button onclick="window.planRoute(${v.lat},${v.lng})">Get Directions</button>
            </div>
        </div>
    `
        });
    ...
    // Highlight active vendor in list
    ...
    window.planRoute = function(destLat, destLng) {
